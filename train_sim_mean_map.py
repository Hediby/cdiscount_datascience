...
if __name__=='__main__':
    ...
    fwrite(' Ok\n')
    pooling = 'mean_map' #mean, max or softmax
    ...
    model = m_sim_mean_map.create(dim_img, dim_txt, dim_multi)
    train_idxs = np.arange(n_train)

    for epoch in range(51):
        if not epoch % 5:
            test_rank = model.test(Xim_test, Xtxt_test, folder, epoch, save=True)
            train_rank = model.test(Xim_train, Xtxt_train, folder, epoch, save=False)
        ...
        tic = time()
        mean_N = model.train(Xim_train, Xtxt_train, gamma, lr_img, lr_txt)
        toc = time()-tic
